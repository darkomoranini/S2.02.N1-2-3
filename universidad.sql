...
SELECT PrimerApellido, SegundoApellido, Nombre FROM Alumnos ORDER BY PrimerApellido, SegundoApellido, Nombre ASC; 
SELECT Nombre, PrimerApellido, SegundoApellido FROM Alumnos WHERE NumeroTelefono IS NULL;
SELECT * FROM Alumnos WHERE YEAR(FechaNacimiento) = 1999;
SELECT * FROM Profesores WHERE NumeroTelefono IS NULL AND RIGHT(NIF, 1) = 'K';
SELECT * FROM Asignaturas WHERE Cuatrimestre = 1 AND Curso = 3 AND GradoID = 7;
SELECT P.PrimerApellido, P.SegundoApellido, P.Nombre, D.Nombre AS NombreDepartamento FROM Profesores P JOIN Departamentos D ON P.DepartamentoID = D.ID ORDER BY P.PrimerApellido, P.SegundoApellido, P.Nombre ASC;
SELECT A.Nombre, C.AñoInicio, C.AñoFin FROM Alumnos Al JOIN Matriculas M ON Al.ID = M.AlumnoID JOIN Cursos C ON M.CursoID = C.ID JOIN Asignaturas A ON M.AsignaturaID = A.ID WHERE Al.NIF = '26902806M';
SELECT DISTINCT D.Nombre FROM Departamentos D JOIN Profesores P ON D.ID = P.DepartamentoID JOIN Imparticiones I ON P.ID = I.ProfesorID JOIN Asignaturas A ON I.AsignaturaID = A.ID JOIN Grados G ON A.GradoID = G.ID WHERE G.Nombre = 'Grado en Ingeniería Informática' AND G.Plan = 2015;
SELECT DISTINCT Alumno.Nombre, Alumno.Apellido FROM Alumno JOIN Matricula ON Alumno.IdAlumno = Matricula.IdAlumno JOIN Asignatura ON Matricula.IdAsignatura = Asignatura.IdAsignatura WHERE Matricula.CursoEscolar = '2018/2019';
SELECT Departamento.Nombre AS 'Nombre Departamento', Profesor.PrimerApellido, Profesor.SegundoApellido, Profesor.Nombre
FROM Profesor LEFT JOIN Departamento ON Profesor.IdDepartamento = Departamento.IdDepartamento ORDER BY Departamento.Nombre, Profesor.PrimerApellido, Profesor.SegundoApellido, Profesor.Nombre;
SELECT Profesor.PrimerApellido, Profesor.SegundoApellido, Profesor.Nombre FROM Profesor LEFT JOIN Departamento ON Profesor.IdDepartamento = Departamento.IdDepartamento WHERE Departamento.IdDepartamento IS NULL;
SELECT Departamento.Nombre AS 'Nombre Departamento' FROM Departamento LEFT JOIN Profesor ON Departamento.IdDepartamento = Profesor.IdDepartamento WHERE Profesor.IdProfesor IS NULL;
SELECT Profesor.PrimerApellido, Profesor.SegundoApellido, Profesor.Nombre FROM Profesor LEFT JOIN Imparte ON Profesor.IdProfesor = Imparte.IdProfesor WHERE Imparte.IdProfesor IS NULL;
SELECT Asignatura.Nombre AS 'Nombre Asignatura' FROM Asignatura LEFT JOIN Imparte ON Asignatura.IdAsignatura = Imparte.IdAsignatura WHERE Imparte.IdAsignatura IS NULL;
SELECT Departamento.Nombre AS 'Nombre Departamento' FROM Departamento LEFT JOIN Imparte ON Departamento.IdDepartamento = Imparte.IdDepartamento WHERE Imparte.IdImparte IS NULL;
SELECT COUNT(*) AS 'Total Alumnos' FROM Alumno;
SELECT COUNT(*) AS 'Alumnos Nacidos en 1999' FROM Alumno WHERE YEAR(FechaNacimiento) = 1999;
SELECT Departamento.Nombre AS 'Nombre Departamento', COUNT(Profesor.IdProfesor) AS 'Número de Profesores' FROM Departamento INNER JOIN Profesor ON Departamento.IdDepartamento = Profesor.IdDepartamento GROUP BY Departamento.Nombre ORDER BY COUNT(Profesor.IdProfesor) DESC;
SELECT Departamento.Nombre AS 'Nombre Departamento', COUNT(Profesor.IdProfesor) AS 'Número de Profesores'
FROM Departamento LEFT JOIN Profesor ON Departamento.IdDepartamento = Profesor.IdDepartamento GROUP BY Departamento.Nombre;
SELECT Grado.Nombre AS 'Nombre Grado', COUNT(Asignatura.IdAsignatura) AS 'Número de Asignaturas'
FROM Grado LEFT JOIN Asignatura ON Grado.IdGrado = Asignatura.IdGrado GROUP BY Grado.Nombre ORDER BY COUNT(Asignatura.IdAsignatura) DESC;
SELECT Grado.Nombre AS 'Nombre Grado', COUNT(Asignatura.IdAsignatura) AS 'Número de Asignaturas' FROM Grado LEFT JOIN Asignatura ON Grado.IdGrado = Asignatura.IdGrado GROUP BY Grado.Nombre HAVING COUNT(Asignatura.IdAsignatura) > 40;
SELECT Grado.Nombre AS 'Nombre Grado', Asignatura.Tipo, SUM(Asignatura.Creditos) AS 'Total Créditos' FROM Grado LEFT JOIN Asignatura ON Grado.IdGrado = Asignatura.IdGrado GROUP BY Grado.Nombre, Asignatura.Tipo;
SELECT YEAR(CursoEscolar.Inicio) AS 'Año Inicio', COUNT(DISTINCT Matricula.IdAlumno) AS 'Alumnos Matriculados' FROM CursoEscolar LEFT JOIN Matricula ON CursoEscolar.IdCursoEscolar = Matricula.IdCursoEscolar GROUP BY YEAR(CursoEscolar.Inicio);
SELECT Profesor.IdProfesor, Profesor.Nombre, Profesor.PrimerApellido, Profesor.SegundoApellido, COUNT(Imparte.IdAsignatura) AS 'Número de Asignaturas' FROM Profesor LEFT JOIN Imparte ON Profesor.IdProfesor = Imparte.IdProfesor GROUP BY Profesor.IdProfesor, Profesor.Nombre, Profesor.PrimerApellido, Profesor.SegundoApellido ORDER BY COUNT(Imparte.IdAsignatura) DESC;
SELECT * FROM Alumno WHERE FechaNacimiento = (SELECT MIN(FechaNacimiento) FROM Alumno);
SELECT Profesor.PrimerApellido, Profesor.SegundoApellido, Profesor.Nombre
FROM Profesor INNER JOIN Departamento ON Profesor.IdDepartamento = Departamento.IdDepartamento LEFT JOIN Imparte ON Profesor.IdProfesor = Imparte.IdProfeso WHERE Imparte.IdImparte IS NULL;
